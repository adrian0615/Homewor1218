...
//
struct Game : Equatable, CustomStringConvertible {
    ...
    //func playGame(row: Int, column: Int, player: Player) -> Game {
    //Game.init(slots: [.empty], lastPlayed: .empty)
    //let winner = Game.isThereAWinner
    //let draw = Game.boardFull
    // while Game.isThereAWinner() == true && Game.boardFull() == true {
    //run Game.takeSlot
    //
}
